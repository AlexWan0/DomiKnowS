...
FIRST_HTML = parts[0].strip().strip('"""').strip("'''").strip()
HTML += markdown.to_html(FIRST_HTML)
...
if parts[-1].strip().startswith('"""') and parts[-1].strip().endswith('"""'):
    ...
    parts = parts[:-1]
elif parts[-1].strip().startswith("'''") and parts[-1].strip().endswith("'''"):
    FINAL_HTML = parts[-1].strip().strip("'''").strip()
    parts = parts[:-1]
else:
    FINAL_HTML = ""
...
for i, comment in enumerate(comments):
    ...
    HTML += f"""<li class="annotation" id="a{i}">{markdown.to_html(comment)}</li>
"""
...
HTML += markdown.to_html(FINAL_HTML)
